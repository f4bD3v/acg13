...
public:
	...
	/// Return whether or not the scene has an environment luminaire
	inline bool hasEnvLuminaire() const { return m_envLuminaire != NULL; }

	/// Return a pointer to the environment luminaire (if any)
	inline const Luminaire *getEnvLuminaire() const { return m_envLuminaire; }
        
        /// Return a pointer to the result evaluator (if any)
        inline const Evaluator *getEvaluator() const { return m_evaluator; }
	...
	inline const std::vector<Mesh *> &getMeshes() const { return m_meshes; }
        ...
        /// Return a reference to an array containing all luminaires
	...
	/// Uniformly pick a luminaire and invoke its direct illumination sampling method
	Color3f sampleDirect(LuminaireQueryRecord &lRec, const Point2f &sample) const;

	/// Compute the density of \ref sampleDirect()
	float pdfDirect(const LuminaireQueryRecord &lRec) const;
