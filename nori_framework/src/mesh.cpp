...
Mesh::Mesh(const PropertyList& propList) : m_vertexPositions(0), m_vertexNormals(0),
  m_vertexTexCoords(0), m_indices(0), m_vertexCount(0),
  m_triangleCount(0), m_bsdf(NULL), m_luminaire(NULL), m_originalTransform(propList.getTransform("toWorld", Transform())) { }
...
Color3f Mesh::getMainColor() const {
        return m_luminaire != NULL ? m_luminaire->getColor() : 
            m_bsdf != NULL ? m_bsdf->getColor() : Color3f(1.0f);
}
	...
	switch (obj->getClassType()) {
		...
		case ELuminaire: {
				...
				if (luminaire->isEnvironmentLuminaire())
					throw NoriException("Mesh: Tried to attach an environment luminaire!");
				m_luminaire = luminaire;
			}
			break;
...
QString Mesh::toString() const {
	...
	.arg(indent(m_luminaire ? m_luminaire->toString() : QString("null")));
}
