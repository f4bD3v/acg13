...
public:
        ...
        Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &_ray) const {
            ...
            TVector<Point3f,1000> positions;
            positions[0]=ray.o;
            positions[1]=its.p;
            Point3f x, xp;
            Vector3f d;
            Color3f Le, f_r;
            float G, dist2, cos1, cos2, q = 0.5f;
            srand (time(NULL));
            for (int depth = 2; depth < 100 ; depth++) {
                // Test  Russian Roulette
                float f = ((float) rand())/((float) RAND_MAX);
                if (depth > 3 && (f < q)){
                    //std::cout <<"depth = " << depth << " & " << f << " < " <<  q << std::endl;
                    break;
                }
                else {
                    if(depth>3){
                    ...
                    if(depth!=2){ // second-to-last point: sampling BRDF to find direction
                        x = positions[depth-3];
                        xp = positions[depth-2];
                        d=(x-xp);
                        d.normalize();
                        BSDFQueryRecord bRec(its.toLocal(d));
                        bsdf->sample(bRec, sampler->next2D());
                        if(!scene->rayIntersect(Ray3f(xp,bRec.wo),its)){
                            break;
                        }
                        positions[depth-1]=its.p;
                        dist2 = (its.p-xp).squaredNorm();
                        cos1 = Frame::cosTheta(bRec.wo);
                        d = its.p-xp;
                        d.normalize();
                        cos2 = Frame::cosTheta(its.toLocal(d));
                        //std::cout<< "cos1 = " << cos1 << ", cos2 = " << cos2 << std::endl;
                        //std::cout << "dist2 = " << dist2 << std::endl;
                        G = /*fabs(cos1) * */fabs(cos2) /dist2;
                        f_r = bsdf->eval(bRec);
                        //std::cout << "bsdf =  " << f_r << std::endl;
                        throughput = throughput * f_r * G;
                        //std::cout << "depth = " << depth << ", G = " << G << std::endl;
                        //std::cout << "depth = " << depth << ", Througput =  " << throughput << std::endl;
                    }
                    ...
                    if ((Le.array() != 0).any()) {
                        d = (x-xp);
                        d.normalize();
                        BSDFQueryRecord bRec(its.toLocal(d),its.toLocal(lRec.d), ESolidAngle);
                        f_r = bsdf->eval(bRec);
