...
class PathTracer : public Integrator {
    ...
    Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &_ray) const {
        ...
        Color3f Le, f_r;
        Vector3f ray_d;
        float G, dist2, cos1, cos2, q = 0.5f, one_q = 1.0f/(1.0f-q);
        double qm = double(q)*RAND_MAX;
        srand (time(NULL));
        for (int depth = 2; depth < 1000; depth++) {
            ...
            if (depth > 3) {
                if (rand() < qm)
                    ...
                    // adapt the throughput once since we play Russian Roulette
                    throughput *= one_q;
            }
            // This iteration we fill positions[depth]
            x = positions[depth-2];
            ...
            if ((Le.array() != 0).any()) {
                ...
                /*if (depth > 2) {///
                    std::cout << "throughput : " << throughput << std::endl<< std::endl;
                    std::cout << "Le : " << Le << std::endl << std::endl;
                    std::cout << "f_r : " << f_r << std::endl << std::endl;
                    std::cout << "evalTransmittance : " << scene->evalTransmittance(Ray3f(lRec.ref, lRec.d, 0, lRec.dist), sampler) << std::endl << std::endl;
                    std::cout << "cosTheta " << std::abs(Frame::cosTheta(bRec1.wo)) << std::endl << std::endl;
            ...
            // Update throughput
            dist2 = (its.p-xp).squaredNorm();
            cos1 = Frame::cosTheta(bRec2.wo);
            ray_d = xp-its.p;
            ray_d = ray_d.normalized();
            cos2 = Frame::cosTheta(its.toLocal(ray_d));
            G = /*cos1 */ cos2 /* dist2*/;
            ///std::cout << "throughput's f_r : " << f_r << std::endl << std::endl;///
            ///std::cout << "throughput's cos1 : " << cos1 << std::endl << std::endl;///
            ///std::cout << "throughput's cos2 : " << cos2 << std::endl << std::endl;///
            throughput *= f_r * std::abs(G) /* bsdf->pdf(bRec2)*/;
        }
        return result;
    }
